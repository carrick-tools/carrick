...
async fn run_analysis_engine_inner<T: CloudStorage + Sync>(
    ...
    if let Some(collected) = crate::analysis_channel::take() {
        ...
        match submitted.job_id.as_deref() {
            ...
            None => logging::finish_spinner(&sp, "Nothing here needs analysing"),
        }
        return Ok(());
    }
...
///
/// Three things can be true here and they are different answers:
///
...
async fn dispatch_analysis_job<T: CloudStorage + Sync>(
    ...
    let dispatched = crate::analysis_job::Dispatched {
        repo: repo.clone(),
        commit: run.commit.clone(),
        job_id: None,
        analyze_rows: collected.rows.len(),
        eta_seconds: None,
    };
    if collected.rows.is_empty() {
        info!("Nothing in {repo} needs the model; there is no job to hand over");
    ...
    let bundle = crate::analysis_job::JobBundle {
        header: crate::analysis_job::JobHeader {
            ...
            counts: crate::analysis_job::JobCounts {
                analyze_file: collected.rows.len(),
