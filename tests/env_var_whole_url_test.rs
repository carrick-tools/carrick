...
/// env-var origin and a keyed lookup for its route could not find it.
#[test]
fn a_whole_url_site_the_extraction_answered_wrongly_is_corrected() {
    let calls = calls();

    let lookup = call_at(&calls, "src/answered.ts", 11);
    assert_eq!(lookup["method"], "POST");
    assert_eq!(
        lookup["target_url"], "${process.env.KNOWLEDGE_URL}/api/lookup",
