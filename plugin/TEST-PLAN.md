...
| 1.5 | npm, Windows x64 | **CI, non-blocking, known red** (carrick#842) | `plugin.yml`, the `install` matrix leg `npm, Windows x64` | the install and `carrick --version` are expected to work; `verdict-probe.mjs` is expected to fail, because `node_modules/.bin/tsc` is a `.CMD` and the spawn has no shell. The leg carries `continue-on-error`, so read its log rather than the tick: its output is the reproduction #842 is missing |
| 1.6 | pnpm install | **CI, non-blocking** (carrick#843) | `plugin.yml`, the `install` matrix leg `pnpm, Linux x64` | `isolation: pnpm` from the probe. The walk has to reach `node_modules/.pnpm/carrick@<v>/node_modules/.bin`; non-blocking until a green run says it does |
| 1.7 | yarn (classic) install | **CI, non-blocking** (carrick#843) | `plugin.yml`, the `install` matrix leg `yarn, Linux x64` | as 1.6 |
