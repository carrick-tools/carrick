...
|---|---|---|---|
| 2.1 | `carrick init` | in a folder of repos, asks which repos this install covers before anything else, every one of them chosen to begin with; then asks nothing until it has an identity; then settles the project (the one they are already in, or one picked from the list); then prints the proposal — the packages found, the repos covered, any project to create, any repo that would move out of the project it is in — and asks. Only after that yes does it write `.carrick/proposal.json` and `.claude/settings.json`, place the repos, and nothing else in the tree. It runs no scan, and ends on the prompt that has an agent write `carrick.json` (carrick#987, carrick#1338). Re-running adds no second hook entry | 0 |
| 2.1c | `carrick init --project <a slug the repos are not in>` in a folder, and Ctrl-C at any question | the project it would move each repo out of is named with its display name and slug, the move is a question of its own, and `--yes` does not answer it. A no, or a Ctrl-C, ends the run: nothing on this machine, and the repos still in the project they started in — check the dashboard, not just the terminal (carrick#1338) | 1 |
...
| 2.14 | No index | `carrick check <file>` from a directory with no `.carrick/` above it | `carrick: no local index for this file. Run \`carrick index --workspace <dir>\` in the folder holding your repos.` on stderr. A refusal from `check` is the one read that exits non-zero: a script could not tell "no contract problems" from "no index" (carrick#1023 item 2). The same command as `touch` still exits 0 | **1** |
| 2.15 | No index, JSON | the same with `--json` | the same stderr line, plus a JSON object on stdout carrying `"error": "not_indexed"` and the schema id. The body is still printed, which is what keeps the hook and the language server answering | 1 |
| 2.16 | File outside the workspace | `carrick check /etc/hosts` | `not_in_workspace` in the same two forms. A refusal, so `check` exits 1 and `touch` exits 0 (carrick#1023 item 2) | **1** |
...
| 2.14 | no index | **pass** | `no local index for this file. Run carrick index --workspace <dir> ...`, exit 0. Recorded before carrick#1023 item 2, which made a `check` refusal exit 1; the row above is the expectation to re-run against |
| 2.15 | no index, `--json` | **pass** | the same line on stderr, `{"schema":"carrick.check/0","error":"not_indexed"}` on stdout, exit 0 — and 1 from carrick#1023 item 2 onwards, with the same body |
